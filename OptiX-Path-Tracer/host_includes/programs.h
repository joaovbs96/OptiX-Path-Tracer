...
void setRayGenerationProgram(optix::Context &g_context, PDF &pdf,
  ...
  optix::Program raygen = g_context->createProgramFromPTXString(
...
void setMissProgram(optix::Context &g_context, Miss_Programs id,
  ...
  if (id == SKY)  // Blue sky pattern
    missProgram =
        g_context->createProgramFromPTXString(embedded_miss_program, "sky");
  else if (id == DARK)  // Dark/Black background
    missProgram =
        g_context->createProgramFromPTXString(embedded_miss_program, "dark");
  else if (id == BOX)  // TODO: implement a proper skybox
    missProgram =
        g_context->createProgramFromPTXString(embedded_miss_program, "box");
  else if (id == IMG) {  // Spherical Environmental Mapping
    missProgram = g_context->createProgramFromPTXString(
        embedded_miss_program, "environmental_mapping");
...
void setExceptionProgram(optix::Context &g_context) {
  optix::Program exceptionProgram = g_context->createProgramFromPTXString(
      embedded_exception_program, "exception_program");
